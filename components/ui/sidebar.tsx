...
type SidebarContextType = {
  ...
  setState: (state: "expanded" | "collapsed" | boolean) => void;
};
...
>(
  (
    ...
    const contextValue = React.useMemo<SidebarContextType>(
      () => ({
        state,
        setState: (stateOrBool) => {
          if (typeof stateOrBool === "boolean") {
            setOpen(stateOrBool);
          } else {
            setOpen(stateOrBool === "expanded");
          }
        },
      }),
      [state, setOpen]
  ...
  (
    ...
    if (isMobile) {
      return (
        <Sheet
          open={state === "expanded"}
          onOpenChange={(open) => setState(open ? "expanded" : "collapsed")}
          {...props}
        >
          <SheetContent
...
>(({ className, onClick, ...props }, ref) => {
  ...
  // Create a stable callback for toggling that won't recreate on each render
  const handleClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      onClick?.(event);
      // Toggle state between expanded and collapsed
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "mb-2 px-4 text-xs font-semibold uppercase tracking-tight",
        className
      )}
      {...props}
    />
